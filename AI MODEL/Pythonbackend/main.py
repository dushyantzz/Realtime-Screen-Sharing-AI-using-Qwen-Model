...
import base64
import torch
from transformers import AutoModelForSpeechSeq2Seq, AutoProcessor, pipeline, Qwen2_5_VLForConditionalGeneration, \
    BitsAndBytesConfig
import numpy as np
import logging
import sys
                    ...
                    if energy > self.energy_threshold:
                        ...
                        if self.silence_counter >= self.silence_samples:
                            ...
                            if len(speech_segment) >= self.min_speech_samples * 2:  # × 2 for 16-bit
                                ...
                                logger.info(
                                    f"Speech segment detected: {len(speech_segment) / 2 / self.sample_rate:.2f}s")
                        ...
                        # Check if speech segment exceeds maximum duration
                            ...
                            self.segments_detected += 1
                            logger.info(
                                f"Max duration speech segment: {len(speech_segment) / 2 / self.sample_rate:.2f}s")
...
class QwenMultimodalProcessor:
    ...
    def __init__(self):
        ...
        # Configure 4-bit quantization with CPU offloading enabled
        ...
        # Reduce image resolution parameters to save memory
        min_pixels = 256 * 28 * 28
        max_pixels = 480 * 28 * 28  # Reduced from default to save memory

        # Load model and processor
        model_id = "Qwen/Qwen2.5-VL-3B-Instruct"
        logger.info(f"Loading {model_id} with 4-bit quantization...")

        # Load model with 4-bit quantization and explicit memory management
        self.model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
            model_id,
            quantization_config=quantization_config,
            device_map="auto",
            max_memory={0: "8GiB", "cpu": "16GiB"}  # Adjust these values based on your hardware
        )

        # Load processor with reduced image resolution
    ...
    async def generate(self, text):
        ...
        async with self.lock:
            try:
                ...
                # Process images and text with reduced dimensions
                inputs = self.processor(
                    text=[text_input],
                    images=[self.last_image],
                    padding=True,
                    return_tensors="pt",
                    resized_height=280,  # Fixed smaller height
                    resized_width=420    # Fixed smaller width
                )
...
async def main():
    ...
    try:
        ...
        logger.info("Starting WebSocket server on 0.0.0.0:9074")
        # Add ping_interval and ping_timeout parameters
        async with websockets.serve(
                handle_client,
                "0.0.0.0",
                9074,  # Change from 9073 to another port like 9074
                ping_interval=20,
                ping_timeout=60,
                close_timeout=10
        ):
            logger.info("WebSocket server running on 0.0.0.0:9074")
